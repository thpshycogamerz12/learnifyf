...
const handleEnroll = async (courseId, userId) => {
  ...
  userId
    ...
    if (verifyRes.data.alreadyEnrolled) {
      ...
      toast.success(verifyRes.data.message || "Payment verified and enrollment successful");
    }
    setIsEnrolled(true);
  } catch (verifyError) {
      ...
      <div className="max-w-6xl mx-auto bg-white shadow-md rounded-xl p-6 space-y-6 relative">
        ...
        <div className="flex flex-col md:flex-row gap-6 ">
          ...
          <div className="flex-1 space-y-2 mt-[20px]">
            ...
            {/* Show manage course button for educators who created this course */}
            {userData?.role === "educator" && selectedCourseData?.creator?._id === userData?._id && (
              <button className="bg-blue-600 text-white px-6 py-2 rounded hover:bg-blue-700 mt-3" onClick={()=>navigate(`/addcourses/${courseId}`)}>
                Manage Course
              </button>
            )}
          </div>
